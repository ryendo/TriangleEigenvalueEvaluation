function val = error_left_hand_side(s,kappa)
    
    kappa = kappa(:)';

    abs_error = (I_intval(I_sup(s))-I_intval(I_inf(s)))*intval(sup(abs(partialF_kappa_s(kappa, s))))./intval(inf(abs(partialF_kappa_kappa(kappa, s))));
...
function val = partialF_kappa_s(kappa, s)
...
function val = partialF_kappa_kappa(kappa, s)
    % partialF_kappa_kappa calculates the partial derivative of F with respect to kappa.
    %
    % Input:
    % kappa - parameter \kappa (vector or scalar)
    % s - parameter s (vector or scalar, must be broadcastable with kappa)
    %
    % Output:
    % val - value of \partial F / \partial \kappa (vector or scalar)
    ...
    % Compute the terms of the partial derivative
    term1 = A1p .* A2p;
    term2 = factor3 .* kappa .* A1 .* A2;

    % Combine terms
    val = c2 .* s .* (term1 + term2);
